...
export default function HeroForm({ mode = "add" }) {
  ...
  const { data, error: fetchError, loading } = useFetchData(
    isEditMode ? `home/gallery/${id}` : null
  );
...
const handleSubmit = async (formData) => {
    ...
    const url = isEditMode ? `home/gallery/${id}` : `home/gallery`;
    ...
    if (formData instanceof FormData) {
      ...
      try {
        ...
        router.push("/admin/home/gallery/list");
      } catch (err) {
  ...
  const formFields = [
    { label: "Sub Text", name: "sub_text", type: "text"},
