...
export default function CTAForm({ mode = "add" }) {
  ...
  const { data, error: fetchError, loading } = useFetchData(
    isEditMode ? `admission/contact/${id}` : null
  );
  ...
  const handleSubmit = async (formData) => {
    ...
    const url = isEditMode ? `admission/contact/${id}` : `admission/contact`;

    try {
      await sendRequest(url, method, formData);
      router.push("/admin/admission/contact/list");
    } catch (err) {
  ...
  const formFields = [
    ...
    { label: "Contact Type", name: "contact_type", type: "text" },
