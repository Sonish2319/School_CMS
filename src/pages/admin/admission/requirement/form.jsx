...
export default function MissionVisionForm({ mode = "add" }) {
  ...
  const { data, error: fetchError, loading } = useFetchData(
    isEditMode ? `admission/requirement/${id}` : null
  );
  ...
  const handleSubmit = async (formData) => {
    ...
    const url = isEditMode
      ? `admission/requirement/${id}`
      : `admission/requirement`;

    try {
      await sendRequest(url, method, formData);
      router.push("/admin/admission/requirement/list");
    } catch (err) {
  ...
  const formFields = [
    { label: "Requirement", name: "requirement_text", type: "textarea" },
  ];
