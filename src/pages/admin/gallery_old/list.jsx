...
export default function GalleryList() {
  ...
  const handleDelete = (id) => deleteItem(`gallery/${id}`, refetch);
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center sm:justify-between bg-white rounded-lg p-3 sm:p-6">
        <div>
          <h1 className="list-title font-light">GALLERY LIST</h1>
          <p className="text-xs text-[#99A6AD]">Manage photo, video, and audio entries</p>
        </div>
...
const GalleryFormWrapper = ({ id, isAdding, onClose, refetch }) => (
  <GalleryForm
    isPopup={true}
    id={id}
    isAdding={isAdding}
