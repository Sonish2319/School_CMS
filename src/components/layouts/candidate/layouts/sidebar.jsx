...
import { myprofile } from "./sidebarapi";
...
export default function Sidebar({ isOpen, setIsOpen }) {
  ...
  const renderNavSection = (title, routes) => (
    <>
      {isOpen && (
        <li className="ms-4 user-select-none uppercase text-sm mb-1 text-gray-400 font-semibold">
          {title}
        </li>
      )}
    ...
    <div className="border-r-[1px] border-gray-300">
      ...
      >
        <aside className="flex flex-col h-full overflow-hidden">
          ...
          {isOpen && userData && (
            <div className="mb-4">
              <UserProfile user={userData} onLogout={handleLogout} userType="candidate"/>
            </div>
          )}
          ...
          <nav className="flex-grow overflow-y-auto">
            <ul>
              {renderNavSection("My Profile", myprofile.slice(0, 4))}
              {renderNavSection("Advertisements", myprofile.slice(4))}
            </ul>
          </nav>
        </aside>
      </div>
